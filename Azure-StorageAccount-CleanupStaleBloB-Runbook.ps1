...
#Define Variables
$subscriptionID = "subscriptionid"   ## Replace subscriptionid with actual subscription id. 
...
Get-AzStorageContainer -context $ctx | select -ExpandProperty name
...
"Count of files identified for deletion:$(($exportdata | where {$_.deletefile -eq $true}).count)"
"Count of files identified to retain:$(($exportdata | where {$_.deletefile -eq $false}).count)"
